...
keys. E12 itself names no consumer and shows none distinguishing any pair — that is **E16's**
contribution, added later against real deployed engines. Nothing here is a security review
...
### The conformance suite — making the residual externally closable (2026-08-12)

The residual above had no mechanism for closing: the rule sequencing existed only as source
code, so anyone wanting to implement it independently had to read the thing they were supposed
to be independent of. `tools/receipt-conformance/` (`@ghost-ark/receipt-conformance` 0.1.0)
removes that obstruction. It ships four things, usable with **zero Ghost-Ark code**, the same
property `kernel-probe` has:

- **SPEC.md** — the decision-receipt verifier rules written to be implementable from the
  document alone, with the provenance warning stated up front: the specification was written
  by this project's authors from their own reference implementation, so conformance to it is
  consistency, not correctness.
- **conformance.json** — a machine-readable contract: 36 cases (3 reproducibility fixtures,
  the 30-fixture corpus with the expected failing check per rejection, and the two RSA-PSS
  digest treatments as separate vectors asserted in **both** directions), canonical-payload
  vectors for self-checking a canonicalizer before any cryptography, and the tenant-commitment
  vector. Generated by `build-conformance.mjs` from the pre-registered manifests, never
  hand-edited; a guard test regenerates and compares on every run.
- **fixtures/** — self-contained copies, byte-identical to the committed originals.
- **run-conformance.mjs** — a harness (Node built-ins only) scoring three levels: `verdict`
  (exit codes, required), `failing-check` (the *declared* check is among the failed ones), and
  `identity` (recomputed `receipt_id`/`digest_sha256` match the committed values). Levels a
  candidate does not expose are reported `not-evaluated`, never counted as passed.

**Command:** `npm run conformance:node` (reference candidate); any verifier honouring the
adapter contract can be swapped in after `--`.

#### Measured result — 2026-08-12, darwin/arm64, Node v22, CPython 3.14.6

| candidate | verdict | failing-check | identity | conformant |
|:---|:---|:---|:---|:---|
| `verifiers/node` (reference) | 36/36 | 30/30 | 4/4 | yes |
| `verifiers/python` | 36/36 | 30/30 | 4/4 | yes |
| `verifiers/thirdparty` (E14 arm) | 36/36 | **27/30** | 4/4 | **no** |

The third row is the suite demonstrating it can see what E14's differential could not. The E14
arm's schema layer is deliberately thin — its job is delegating cryptography, not
reimplementing the field contract — so on `MAL-019` (policy_hash removed), `MAL-023` (extra
field), and `MAL-026` (unsupported schema_version) it rejects via an **incidental**
`receipt_id` mismatch rather than the declared `schema` check. Verdict-level agreement, which
is all E14 measures, conceals exactly this class of rule difference; the failing-check level
reports it. The mismatch is pinned by a test rather than fixed, because "the arm rejects for
the wrong reason" is a *finding about what verdict agreement hides*, and repairing it would
delete the evidence while the question "should a crypto-delegation arm carry the full field
contract?" is a design decision, not a bug.

Guards (`tests/unit/conformance/conformanceArtifact.test.ts`): committed artifact must match a
fresh generation (`--check`); the shipped runner must import Node built-ins only; the reference
verifier must conform on all three levels; four mutants must each be caught (accept-everything
→ 30 verdict mismatches, reject-everything → 6, wrong-reason → 30 failing-check mismatches,
wrong-identity → 4 identity mismatches); a missing fixture must abort the run rather than
shrink the suite; and the third-party arm's three mismatches are asserted exactly.

**What this changes about open gap #10, stated precisely: the gap now has a closing mechanism
and remains open.** A conformance suite written by this project cannot close a gap about
independence from this project — but an implementation produced from SPEC.md alone, under the
blindness protocol recorded in `tools/receipt-conformance/BLINDNESS_PROTOCOL.md` *before any
such implementation exists*, would. Until one does, every claim stays labelled as E14's are:
narrows, does not close.
...
## E16 — Does a deployed, named consumer distinguish a pair the canonicalizer collapses?

**The question this closes.** Every `distinct` intent in the E1 alphabet is a *declared*
consumer model — "an auditor would care", "a ledger distinguishes these". None was ever an
*observed* one. After E12 confirmed F2, 00_THESIS.md named this as the single largest open
weakness: **no consumer had been named and shown to distinguish any pair.** E1, E11, and E12
are all about the *structure* of the kernel; the thesis needed one measurement about its
*consequence* — a real, deployed decision engine reaching two outcomes on two documents the
receipt maps to one identity.

**Design.** Take pairs the Ghost-Ark receipt canonicalizer collapses (verified in-process:
`sha256(canonicalize(parse(A))) === sha256(canonicalize(parse(B)))`), and feed BOTH raw
documents to real, version-pinned decision engines the project did not write. Record each
engine's DECISION, not its parse. The policy is uniform and derived from the receipt itself:
**accept iff `field == R`, where `R` is the single value both documents resolve to under V8** —
i.e. the value the receipt identity actually commits to. It is the most natural policy a
consumer who *trusts* the receipt would write, not one gerrymandered to the pathology.

**Why this is a decision, not a parse.** The trap is counting a re-serialization echo as a
decision: `opa eval 'input.v'` prints `1.50` for one document and `1.5` for the other, but that
is the engine echoing wire bytes. So the decision is always *semantic numeric equality* against
`R`, and the experiment includes a **discriminator** — `trailing-zero-fraction`, `{"v":1.50}`
vs `{"v":1.5}`, which the receipt also collapses and which every engine MUST treat identically
because 1.50 and 1.5 are the same number. If any engine flips on it, the harness is reading
bytes rather than decisions and refuses to certify the run. CPython is a second, in-band
discriminator: it collapses `decimal-literal-collapse` (IEEE-754 double) and so must NOT
distinguish it, even where OPA/CUE/jq do.

**The four engines, pinned.** None is a Ghost-Ark component; the last two are disclosed as also
serving elsewhere as canonicalizer arms (E1/E11), so they are not independent of the repository.

| engine | version | decision kind | independent of this repo |
|:---|:---|:---|:---|
| Open Policy Agent (Rego) | OPA 1.19.0 | `default deny; allow if field == R` | yes |
| CUE | v0.17.1 | `cue vet` schema conformance (`field: R`) | yes |
| jq | 1.7.1-apple | `jq -e 'field == R'` (exit code is the decision) | no (E11 arm) |
| CPython `json` | 3.14.6 | `json.load` then `field == R` | no (E1/E11 arm) |

**Provenance: census.** Curated pairs, exact engine decisions, no confidence intervals. This is
an existence result, not a rate.

**Command:** `npm run experiment:e16` (`-- --json` for the artifact, `-- --allow-degraded` to
run with an engine missing). Emits `artifacts/e16-consumer-distinguishability.json`.

### Measured result — Apple M1, darwin/arm64, Node v22.22.3

Each finding pair is one receipt identity (the shared digest is the same for A and B). The cell
records the engine's decision on side A and side B.

| pathology (intent) | shared receipt id (sha256) | OPA | CUE | jq | CPython |
|:---|:---|:---|:---|:---|:---|
| `decimal-literal-collapse` (distinct) | `1b8466c9…` | accept→**deny** | conform→**conflict** | pass→**deny** | accept→accept |
| `integer-precision-loss` (distinct) | `a2102c7f…` | **deny**→accept | **conflict**→conform | **deny**→pass | **deny**→accept |
| `duplicate-key-last-wins` (distinct) | `a2879a37…` | accept→accept | **conflict**→conform | accept→accept | accept→accept |
| `trailing-zero-fraction` (**discriminator**) | `bac0a59f…` | accept→accept | accept→accept | accept→accept | accept→accept |
| `float-vs-integer-same-value` (equivalent) | `afbf9d0f…` | accept→accept | **conflict**→conform | accept→accept | accept→accept |
| `numeric-exponent-form` (equivalent) | `f5e21757…` | accept→accept | **conflict**→conform | accept→accept | accept→accept |

- `{"rate":0.1}` vs `{"rate":0.1000000000000000055511151231257827}` — one receipt id, three engines split.
- `{"amount":9007199254740993}` vs `{"amount":9007199254740992}` — one receipt id, all four split.
- `{"amount":1,"amount":2}` vs `{"amount":2}` — one receipt id, only CUE splits.

```
ONE RECEIPT IDENTITY, TWO CONSUMER OUTCOMES:   8 cells
distinguishing consumers (closes the gap):     opa-rego, cue, jq, cpython
DISCRIMINATOR (every engine agrees on 1.50 vs 1.5):  HOLDS
```

### Findings

**F16.1 — The gap is closed by execution.** OPA/Rego 1.19.0, a CNCF-graduated policy engine,
returns `allow` for `{"rate":0.1}` and `deny` for `{"rate":0.1000000000000000055511151231257827}`
under the policy `allow if input.rate == 0.1`. Both documents parse — neither is rejected — and
the Ghost-Ark receipt assigns them the *same* identity `1b8466c9…`. One receipt identity, two
authorization outcomes. Every `distinct` intent in this repository was a declared consumer model;
this is the first that is observed, on a real deployed engine, by running it.

**F16.2 — Three independent engines split the decimal-literal pair; a fourth does not, and that
is the point.** OPA, CUE, and jq each distinguish `0.1` from `0.1000…0555`; CPython collapses
them to one IEEE-754 double and correctly returns the *same* decision on both. A harness that
always reported a flip would be a bug. The negative control inside the finding set proves the
flip tracks each engine's number model, not the wire bytes.

**F16.3 — The discriminator holds, so the decisions are semantic.** On `trailing-zero-fraction`
every engine accepts both sides, because 1.50 and 1.5 are the same number to all of them. The
harness refuses to certify a run where any engine flips on this pair. A guard test injects a
wire-byte consumer and confirms the discriminator can go false — a discriminator that cannot
fail certifies nothing (the E4 principle, applied to E16 itself).

**F16.4 — The universal duplicate-key collapse is INERT for last-wins consumers and FATAL for
CUE.** `{"amount":1,"amount":2}` and `{"amount":2}` are collapsed by every canonicalizer (E11),
and OPA, jq, and CPython all resolve the duplicate last-wins, so the collapse changes no decision
for them. CUE treats the repeated field as a unification conflict and rejects the duplicate
document. So whether a real, universal kernel member has any consequence depends entirely on the
consumer — which is `Sound(C, Σ, P)` depending on `P`, measured on deployed software.

**F16.5 — CUE over-discriminates two `equivalent`-intent pairs, and it is reported as the
antitone dual, not as an unintended kernel member.** CUE's type system separates int from float,
so it splits `{"v":1.0}` from `{"v":1}` and `{"v":1e2}` from `{"v":100}` — pairs Ghost-Ark
declares equivalent and collapses (E1 scores them *sound*). This is a consumer whose model is
*finer* than the declared equivalence, the same phenomenon E7 saw as a portability hazard in
canonical form, now surfacing as a schema-validation decision. It challenges an `equivalent`
pre-registration rather than confirming a `distinct` one, and is tabulated separately for that
reason.

**F16.6 — What this does and does not establish, in one sentence each.** It establishes that a
divergence between one receipt identity and a real consumer's decision is not hypothetical: named,
deployed, version-pinned engines exhibit it on execution. It does **not** establish that such a
divergence occurs in production traffic — E12 measured that and found 0 of 64 — so the thesis
moves from "structurally, soundness is consumer-relative" to "consequentially, a real consumer's
decision *can* diverge on a collapsed pair", and stops there. Possible, demonstrated; not
prevalent, and not claimed to be.

### Coverage boundary (what E16 does NOT establish)

Four engines, one host, one version each. The pairs are scalar single-field documents so the
`== R` policy is well-defined; the structural duplicate-key variants (`nested-duplicate-key-in-array`,
`duplicate-empty-key`) are analogous but out of scope for a scalar value policy and are not
measured here. The policy is one this experiment wrote; a different policy would decide
differently, and the finding is that *a* natural policy suffices, not that every policy does.
Above all, **this is an existence result, not an incidence rate** — it says these engines CAN
diverge on a collapsed pair, and says nothing about how often anyone deploys such a consumer
against such traffic. It is not a security review of OPA, CUE, jq, or CPython; each behaves
exactly as documented, and a "flip" means only that the engine's number model is finer than the
receipt's canonical form — a property of the receipt, not a defect in the engine.
...
1. ~~**No real-traffic corpus.**~~ CLOSED by E12, and the closure went **against** the
   ...
   What replaced it as the largest open weakness was narrower and sharper —
   *no consumer had been named and shown to distinguish any pair* — and **that is now
   CLOSED by E16.** OPA/Rego 1.19.0, CUE v0.17.1, and jq 1.7.1 each reach two different
   decisions on `{"rate":0.1}` vs `{"rate":0.1000…0555}` — one Ghost-Ark receipt identity,
   two consumer outcomes, both documents parsed. This converts the finding from structural
   to consequential. It does **not** make it prevalent: E16 is an existence result on
   deployed engines, and E12's 0/64 for real traffic stands alongside it. The remaining
   open form of this weakness is incidence, not existence — see gaps #2 and #3.
...
10. **No third-party reimplementation** — narrowed by E14, not closed. E5 reports agreement
   ...
   the specification, fixes that, and none exists. **Since 2026-08-12 the gap has a closing
   mechanism**: `tools/receipt-conformance/` publishes the rule sequencing as a specification
   plus a 36-case machine-checkable suite usable with zero Ghost-Ark code (§E14, conformance
   subsection), and `BLINDNESS_PROTOCOL.md` records — before any such implementation exists —
   what an independent implementation must and must not have seen for its agreement to count.
   The gap itself stays open until somebody outside this project writes one.
