...
\author{Pranav Bhave\\ \normalsize\texttt{bhavepranavwork@gmail.com}}
...
effectful tools. The dominant defense --- semantic guardrails --- emits
probabilistic judgments about \emph{content}, but enforcement is a question
about \emph{state}: which bytes may mutate the environment, under which
recorded justification. We present \ghostark{}, a control plane that treats an
agent trajectory as a software transaction. An agent executes speculatively
against a ghost replica $G(\sigma_0)$; a separate trusted gateway admits its
effects only through a three-gate validation pipeline --- a \emph{ledger gate}
(nonce freshness with spent-tombstone semantics), an \emph{OCC gate}
(hash equality over a read-set projection $\piR$, which restores liveness lost
to global-state validation), and a \emph{semantic gate} (a Fr\'echet
bound on cumulative trajectory failure that assumes nothing about independence
between steps). Every admitted or aborted transaction yields a signed,
canonically serialized, independently replayable receipt.

We report what is implemented and measured, and label what is only specified.
TLC model checking of five bounded models (up to \plstates{} distinct states)
is clean, and five seeded mutants reproduce their intended violations ---
including a true-positive replay flaw that model checking exposed in our own
garbage-collection design, whose repair we then propagated to the Rust
gateway. Against a nine-attack adversarial corpus and four security games
(\benchtrials{} trials each), the modeled attacker's advantage is
\globaladvantage{}. The in-process enforcement path adds
$\approx\addedmean{}\,\mu$s mean latency ($p50=\eepfifty{}\,\mu$s end to end)
on commodity hardware. \ghostark{} verifies what was recorded, signed,
policy-bounded, and replayable; it does not certify that any agent output is
true, aligned, or semantically safe.
\end{abstract}
...
The deficit is structural, and it has a database-shaped description. A typical
...
\emph{fails closed} --- in this design a silent compromise is definitionally
an unrecorded egress, and unrecorded egress does not execute. \statusimpl{}
...
The discipline caught us. The original nonce-ledger baseline --- once a
spec-level operator defect was fixed and it parsed at all --- \emph{violated
its own} \texttt{NoReplays} \emph{invariant}: \texttt{GarbageCollect}
removed consumed nonces from the ledger, so a second agent could re-consume
a garbage-collected nonce. TLC produced the counterexample; the flaw was a
true positive about the shipped design, not the model. The repair introduced
the spent-tombstone set (consumption requires
$n \notin \mathcal{L} \wedge n \notin S_{\mathrm{spent}}$; GC archives
rather than forgets), after which TLC verifies \texttt{NoReplays} and the
temporal property \texttt{EventualGC} over the complete bounded state space.
The Rust gateway initially retained TTL-eviction semantics --- diverging
from the verified model by exactly the refuted behavior --- and was then
brought into conformance: \texttt{cleanup\_expired()} archives into a spent
set, and a nonce consumed at $T$ is rejected at $T+3601$. One bounded
caveat survives and is stated in \S\ref{sec:nonclaims}: the in-process
tombstone set is capacity-bounded, and pruning at capacity reopens a
theoretical replay window for sufficiently old nonces.

We report this episode at length because it is the paper's methodological
argument in miniature: the value of the formal layer was not a certificate
but a \emph{refutation}, caught before deployment, with the repair
propagated spec-first into the implementation and both directions of the
oracle (clean baseline, violating mutant) kept as regression evidence.
