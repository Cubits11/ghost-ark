...
% Test suite at HEAD (make unit, 2026-07-17): 706 tests / 105 files, all passing
\newcommand{\testcount}{706}
\newcommand{\testfiles}{105}
% Claim-language gate at HEAD (npm run scan:claims, 2026-07-17): 600 scannable
% files on a fresh clone, 0 hits — coverage includes .tex/.bib (this manuscript).
% Count is the tracked tree; the scanner also walks gitignored generated
% artifacts, so a post-reproduce working tree reports a few more.
\newcommand{\claimfiles}{618}
...
\emph{detected} in-suite, and the four security games report attacker
advantage \globaladvantage{} over \benchtrials{} trials each
(Table~\ref{tab:games}) ($\Pr[\text{win}] = 0$ across
...
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Security game} & \textbf{Trials} & \textbf{Adversary advantage}\\
\midrule
Mutation resistance        & \benchtrials{} & 0 \\
Replay resistance          & \benchtrials{} & 0 \\
Cross-transaction binding  & \benchtrials{} & 0 \\
Serialization ambiguity    & \benchtrials{} & 0 \\
\bottomrule
\end{tabular}
\caption{Formal security games at HEAD (recorded run, 2026-07-16). Advantage
is the fraction of trials the modeled attacker wins. In-suite results under
the modeled attacker; the non-claim header of \texttt{run\_all.ts} is
normative.}
\label{tab:games}
\end{table}
...
\textbf{Concurrent TCB throughput, measured.} Table~\ref{tab:perf} times the
\emph{TypeScript} commitment cycle --- canonical serialization plus SHA-256
digest and comparison; it performs no signature, and its ops/s column is
single-thread inverse latency, not a load test. We therefore also measure the
...
and receipt schema, the Rust gateway sources, five TLA+ models with five
mutants and their recorded TLC logs, the nine-attack adversarial corpus and
four security games, the performance harness, the claim-language scanner
...
\item \textbf{Hardware:} any commodity machine; the paper's latency numbers
  are from an Apple M1 (8\,GB) and reproduce within machine variance.
\item \textbf{Metrics:} attacker advantage, detection flags, TLC
  distinct-state counts, latency percentiles, test counts, claim-gate status.
\item \textbf{Expected wall-clock:} proofs $\sim$15--25\,s; corpus + games
  $\sim$30--60\,s; full unit suite $\sim$2.5--3.5\,min;
...
\item Table~\ref{tab:games} and the nine-attack corpus:
  \texttt{node -{}-experimental-strip-types dab/bench/run\_all.ts
  -{}-trials 10000} $\rightarrow$ \texttt{global\_advantage: 0},
  \texttt{all\_passed: true}. Exact match.
\item Table~\ref{tab:perf}: same command (the \texttt{performance} block) or
  \texttt{make benchmark}. Microsecond scale reproduces; exact values are
  machine-dependent.
