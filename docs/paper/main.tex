...
% suite as one.
\newcommand{\testcount}{1{,}274}
\newcommand{\testfiles}{165}
...
\author{
  Pranav Bhave \\
  \normalsize\href{https://github.com/PSUCyberSecurityLab/ghost-ark/}{\color{blue}github.com/PSUCyberSecurityLab/ghost-ark} \vspace{0.2em} \\
  \normalsize Correspondence via the repository's issue tracker until an
  institutional address is set.
}
...
\item \textbf{Model checking with teeth} (\S\ref{sec:tla},
  \S\ref{sec:eval-tla}): five TLC-checked bounded models plus four seeded
  mutants that must fail (the fifth model has none, and we say so) ---
...
with a README stating it is not evidence about this system and a test
asserting no workflow treats it as such.
...
state space, and all five re-derived byte-identically across a toolchain change.
...
Five baselines, \emph{four} mutants: \texttt{DAB\_ExecutionBoundary} has no
seeded mutant, so its clean result is one-sided. These are bounded models of
the design, not proofs of the implementation.}
\label{tab:tlc}
\end{table}

Table~\ref{tab:tlc} summarizes five baselines and four mutants. The
...
teeth rather than as a fifth mutant. A sixth model,
\texttt{TenantIsolation}, is a declared stub: excluded from the gate rather than
passed vacuously.
