...
\subsection{The gateway$\leftrightarrow$verifier round-trip}
The Rust gateway and the independent verifier now agree on a real ed25519
...
in-cluster). Scope: a local DEV ed25519 key (not KMS/HSM/TPM/Nitro). Two
residuals are stated in the repository inventory and not claimed here: the
untrusted agent's Unix-socket transport (the agent runtime has no entrypoint)
and wiring the TLC-verified tombstone module into the running gateway binary
(it is currently orphaned; the binary uses an inline monotonic ledger).
