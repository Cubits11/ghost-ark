...
% Claim-language gate at HEAD (npm run scan:claims): 589 scannable files, 0 hits
\newcommand{\claimfiles}{589}
...
\emph{fails closed} --- any compromise that bypasses the gateway cannot produce side-effects, guaranteeing that all egress is evidenced. \statusimpl{}
...
\begin{enumerate}
    ...
    \item \textbf{Speculative Collapse:} The transaction fails the Semantic Gate's lattice policy (A-CC violation).
    \item \textbf{Evidence Generation:} The gateway discards the speculative buffer, preventing the network request. It emits a canonical JSON receipt containing the exact payload, a \texttt{COLLAPSE} status, the breached lattice policy, and a cryptographic signature. The compromise is structurally contained and permanently evidenced.
\end{enumerate}
...
While this paper evaluates the in-process enforcement baseline, \ghostark{}'s architecture maps directly to standard cloud infrastructure. In a production deployment, the untrusted agent runtime (e.g., a LangChain/ReAct loop hitting Amazon Bedrock) runs in an isolated container. The \ghostark{} Gateway operates as an out-of-process sidecar or AWS Lambda function.

The speculative buffer $G(\sigma_0)$ is maintained locally by the gateway. At commit time, the ledger and OCC gates resolve against a fast KV store (e.g., DynamoDB) using conditional writes to enforce serializability across concurrent agent sessions. Finally, receipt signing is offloaded to AWS KMS (using asymmetric ECDSA or RSA). While cloud I/O will dominate the $\approx\addedmean{}\,\mu$s local arithmetic costs reported in \S\ref{sec:eval-perf}, the transactional guarantees remain structurally identical.
