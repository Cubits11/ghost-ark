...
### 7.1 DAB TLA+ specs are invalid TLA+
`proofs/dab/DAB_NonceLedger.tla` and `_Mutant.tla` use LaTeX `\setminus` where
TLA+ set-difference is a bare `\`. Under TLC they fail:
`Parse Error … Encountered "{" … token "setminus"`. They have never parsed, so the
TLC trace transcribed in `docs/dissertation/06_Formal_Verification.md` and the
`09_Artifact_Evaluation_Appendix.md` reviewer command cannot have been produced by
these files.

### 7.2 The corrected DAB baseline refutes its own claim
Fixing **only** the operator (`\setminus`→`\`) makes the specs parse, and the
baseline then **violates `NoReplays`** (TLC: 169 distinct states). `GarbageCollect(n)`
removes a consumed nonce, permitting a second agent to re-consume it — two distinct
agents reach `Executed` with the same nonce. The dissertation claims the model shows
"mathematical immunity to replays." It does not. **This pipeline does not edit the
spec to force a pass** — doing so would fabricate the result. It quarantines the DAB
proof as UNVERIFIED and leaves the modeling decision to the author.
