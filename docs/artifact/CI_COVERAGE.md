...
| **The pinned `tla2tools.jar` hash does not match the asset** | `scripts/run-proofs.sh` pins `58d44845…`; the v1.8.0 release URL currently serves `e22f8ffb…`; the untracked jar at repo root is a third artifact, `cc4803dc…`. The proof stage of `make reproduce` therefore fails closed and reports `Proofs Passed: 0/0`. | **Deliberately not "fixed" by re-pinning to the observed hash** — that converts an integrity check into a rubber stamp, and this repository already carries one documented case of a threshold declared rather than met. Resolving it needs the upstream question answered: was the v1.8.0 asset re-uploaded, or was the pin wrong when recorded? Until then the stage stays red. |
| **`artifacts-verify` runs TLA+ from an UNPINNED download** | The `tla-proofs` job fetches the same `tla2tools.jar` with plain `curl` and no hash check, so it passes while the pinned path in `make reproduce` refuses. The TLA+ results CI reports as green come from an unverified jar. | This inconsistency is the sharper half of the row above: one workflow verifies the toolchain and one does not. Both should use the same pinned verification once the correct hash is established. |
