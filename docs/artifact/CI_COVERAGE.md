...
| Claim-language discipline | `scan:claims` over 824 files, 0 violations required | `ci.yml` | yes: forbidden vocabulary fails the build |
...
| **The kernel's mutation score is 81.0%, and 10.1% of its mutants are unreached** | All ten declared files measured (2026-08-02): aggregate 81.0% covered (1170/1445), 72.8% on Stryker's total denominator, 275 survivors, **163 mutants executed by no test at all**. `kmsVerifier.ts`, `emission.ts`, and `chain.ts` were remediated from 48.1/56.3/81.7% to 93.9/98.1/95.4%, taking their unreached mutants from 101 to 3. The weakest remaining are `canonical.ts` (61.0%), `keyManifest.ts` (61.0%), and `signer.ts` (61.4%). | The gate is `break: 70`, set from the measured 72.8% — it has been wrong once already (`75` chosen on two files' evidence when the full sweep held 60.6%) and now moves only after a sweep says it can. Full table, per-file triage, and the equivalent-mutant arguments in EXPERIMENTS.md §E10. |
