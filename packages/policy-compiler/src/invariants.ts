...
export function verifyTenantSandboxPolicyInvariants(input: {
  ...
  for (const statement of input.document.Statement) {
    ...
    if (effect === "Allow" && actions.includes("*")) {
      violations.push({ code: "allow_wildcard_action", detail: "Allow statements cannot grant Action '*'.", sid });
    }
    ...
    if (effect === "Allow" && referencesReceiptTable) {
      const forbidden = actions.filter((action) => RECEIPT_LEDGER_FORBIDDEN_ACTIONS.has(action));
